package com.taoxue.utils.glide;
...
import com.taoxue.R;
import com.taoxue.utils.UtilDisplay;

import jp.wasabeef.glide.transformations.BlurTransformation;

import static com.taoxue.utils.UtilTools.getStringEND;
...
//                .into(imageView);
    ...
    public static void showImageViewBlur(final Context context, int errorimg,
                                         String url, final ImageView imageView) {
        Glide.with(context).load(getStringEND(url)).asBitmap().error(errorimg).centerCrop()
                // 设置错误图片

                .diskCacheStrategy(DiskCacheStrategy.RESULT).dontAnimate()
                // 缓存修改过的图片
                .placeholder(errorimg)
                .transform(new BlurTransformation(context, 14, 3))// 高斯模糊处理
                // 设置占位图
                .into(imageView);
